...
fish <- left_join(fish, extr, by = "sample_id") %>% 
  select(sample_id, extraction_id, status)

fish <- fish %>% 
  mutate(status = ifelse(!is.na(extraction_id), "extracted", "need extraction"))

# get all of the extraction ids of samples that have been digested
digs <- lab %>% 
  tbl("digest") %>% 
  select(extraction_id, digest_id) %>% 
  collect() 

fish <- left_join(fish, digs, by = "extraction_id") %>% 
  select(sample_id, extraction_id, digest_id, status)

fish <- fish %>% 
  mutate(status = ifelse(!is.na(digest_id), "digested", status))



# take a look at the sample that haven't been digested
fish 

  
```
The groups of status are:
```{r status, echo=FALSE}
x <- fish %>% 
  group_by(status) %>% 
...
kable(x) %>% 
  kable_styling()
```
The samples that have been extracted but not digested are:
```{r non-digest, echo=FALSE}
no_dig <- fish %>% 
  filter(status == "extracted")

kable(no_dig) %>% 
  kable_styling()
```
Some of these are samples that have been assigned an extraction_id but have not been extracted yet.

Some of these are samples that can't be extracted because of poor sample quality or empty wells.

```{r get_notes, echo=FALSE}
notes <- lab %>%
  tbl("extraction") %>% 
  filter(extraction_id %in% no_dig$sample_id) %>% 
  select(extraction_id, notes) %>% 
  collect()

no_dig <- left_join(no_dig, notes, by = c("extraction_id"))
```

There are no notes in these columns, hmmmm.
