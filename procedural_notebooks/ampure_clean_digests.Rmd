...
1. Calculate the volumes of reagents needed.  The best way to do this is to use the clean tab on the [PCR worksheet](https://docs.google.com/spreadsheets/d/1LGt2WziwmGoJMluBcwmKfhZrGjZOlJLkVHf59-5cOV4/edit?usp=sharing) or in the R code below
...
setup <- data.frame(NA)
setup <- setup %>% 
  mutate(number_samples = x, 
    error = 1.1 * x, 
    volume_sample = 50, 
    volume_ampure = 1.5 * volume_sample, 
    total_ampure = volume_ampure * error,
    total_EtOH = 400 * error) %>% 
  select(-NA.)
kable(setup) %>%
  kable_styling() %>%
  add_header_above(c("Clean" = 6))
```
...
print(y)
```
volume of ampure per sample
```{r echo = F}
a <- y * 1.5
print(a)
```
total volume of ampure needed
```{r echo = F}
a*z
```
total volume of 70% EtOH needed
```{r echo = F}
400 * x
```
...
print(b)
```
How many boxes of 100uL pipets?
```{r echo = F}
5*b
```
How many boxes of 300uL pipets?
```{r echo = F}
2*b
```
