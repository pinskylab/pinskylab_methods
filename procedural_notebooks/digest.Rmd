...
library(kableExtra)
```
```{r create_tables, echo=FALSE, message=FALSE}
setup <- data.frame(NA)
setup <- setup %>% 
  mutate(number_samples = params$num_samples, 
    error = 1.1 * number_samples, 
    reaction_size = 50, 
    enzyme1_conc = 100000, 
    enzyme2_conc = 10000, 
    mstr_mx_ppt_vol = 20, 
...
per_sample <- per_sample %>% 
  mutate(uL_sample = 30, 
    ul_enzyme1 = (setup$unts_ech_enzyme/50 * setup$reaction_size)/(setup$enzyme1_conc/1000), 
    ul_enzyme2 = (setup$unts_ech_enzyme/50 * setup$reaction_size)/(setup$enzyme2_conc/1000), 
