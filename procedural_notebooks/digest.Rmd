...
setup <- setup %>% 
  mutate(number_samples = num_samples, 
    ...
    enzyme1_conc = 100000, 
    enzyme2_conc = 10000, 
    master_mix_pipet_vol = 20, 
    num_units_each_enzyme = 20) %>% 
  select(-NA.)
kable(setup) %>% 
  kable_styling() %>% 
...
per_sample <- per_sample %>% 
  mutate(uL_sample = 30, 
    ul_enzyme1 = (setup$num_units_each_enzyme/50 * setup$reaction_size)/(setup$enzyme1_conc/1000), 
    ul_enzyme2 = (setup$num_units_each_enzyme/50 * setup$reaction_size)/(setup$enzyme2_conc/1000), 
