...
recaps <- clown %>% 
  filter(tag_id %in% multi$tag_id & recap != "Y") %>% 
  arrange(tag_id)

# this fish was captured and the tag was scanned but the fish escaped before the tag could be implanted so the next fish got the tag
# make sure clown is fresh from db
clown <- leyte %>% 
  tbl("clownfish") %>% 
  collect()
