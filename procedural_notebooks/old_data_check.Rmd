...
recaps <- left_join(recaps, select(anem, anem_table_id, dive_table_id, obs_time))
# attach date
recaps <- left_join(recaps, select(dive, dive_table_id, date)) 
recaps <- recaps %>% 
  select(-dive_table_id)

# look at the pitscan table for those times
test <- pit %>% 
  filter(date %in% recaps$date) %>% 
  arrange(date, time)
