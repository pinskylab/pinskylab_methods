...
params:
  file: ~/Downloads/2018-07-31-full.txt
  id: digest_id
  first_sample: D5676
  last_sample: D6431
  type: digest
...
kable(check) %>% 
  kable_styling()
```
Some of the rows from D6252-D6347 have been assigned to D6348-D6431.  Why?
```{r message=FALSE}
dat <- dat %>% 
  mutate(plate = ifelse(Sample >= 673 & Sample <= 677, "D6252-D6347", plate))
```
Rerun the block above that gets the data into the db table with the plate assignment commented out.  Look over the table to make sure things look correct.
