...
plate <- data.frame(row = rep(LETTERS[1:8], 12), col = unlist(lapply(1:12, rep, 8))) %>% 
  mutate(sample_id = ifelse(row == "G" & col == 2, "XXXX", NA), #normally this one is D2 but I forgot to add the blank until after some fins were loaded.
...
0,0,0") %>% 
mutate(num_samples = w_error, 
ul_prot_k = w_error * 20 * 0.001, 
ml_ATL_buff = w_error * 180 * 0.001)

kable(mix)
```
###Set up tubes of fin clips into the plate formation on tube holder block according to plate map.

###Create the lysis mix in a 50mL falcon tube, **do not vortex (foamy)**, and pour into 50mL reservoir.

###Place an 8 well strip of collection tubes (H position labeled with the column number) onto the loading plate and fill with 200uL of lysis mix.

###Double check that the next column of tubes matches the plate map

###Place the fins in the collection tubes

###Cap the tubes

###When the entire plate is done, place in the incubator overnight.
####