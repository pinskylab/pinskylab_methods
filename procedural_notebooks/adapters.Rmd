...
x <- 51.4 * 5
   print(paste("Add ", x, "uL of 1xTE buffer to the X.1 IDT tube.", sep = ""))
   # P2.2 from 2016-04-26 has 78.6nmoles
(x <- 78.6 * 5)
   print(paste("Add ", x, "uL of 1xTE buffer to the X.2 IDT tube.", sep = ""))
}
```
...
```{r P1mix, echo=FALSE}
if (params$working == TRUE){

  print("To make P1 working stock:")
...
print(" 1. Prepare 2 centrifuge tubes of **1x annealing buffer **
...
P2mix <- P2mix %>% 
  # multiply number of samples by 1.1 for pipetting error
   mutate(
   vol = ifelse(ingredient == "P2_adapter_stock", round(params$vol_P2/C20) , vol),
   vol = ifelse(ingredient == "1x_annealing_buffer", params$vol_P2 - as.numeric(P2mix$vol[1]), vol)
   )
...
kable(P2mix) %>%
  ...
  print("## 1. Add the above amount of  **1x annealing buffer ** to a centrifuge tube.  
## 3. Add the above amount of annealed P2 adapter.  
