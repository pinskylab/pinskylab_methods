...
liftons <- readr::read_csv("ingredient, initial_conc, initial_vol, final_conc, final_vol
mM_EDTA, NA, NA, 100, NA
mM_Tris-HCl, NA, NA, 25, NA
perc_SDS, NA, NA, 1, NA
") %>% 
  mutate(final_vol = 1200*1.1*params$num_samples)
kable(liftons) %>%
  kable_styling() %>%
...
uL_Liftons, NA, 40, NA, NA
mM_DTT, NA, NA, 75, NA
mg/ml_ProteinaseK, NA, NA, 4.2, NA
") %>% 
  mutate(final_vol = 40*1.1*params$num_samples)
kable(enzyme) %>%
  kable_styling() %>%
  add_header_above(c("Enzyme Solution" = 5))
```
**Prepare hybridization solution**
```{r hybrid, echo=FALSE}
hybrid <- readr::read_csv("ingredient, initial_conc, initial_vol, final_conc, final_vol
M_NaCl, NA, NA, 2.5, NA
perc_PEG_8000, NA, NA, 20, NA
mM_DTT, NA, NA, 25, NA
") %>% 
  mutate(final_vol = 45*1.1*params$num_samples)
kable(hybrid) %>%
  kable_styling() %>%
...
mM_Tris-HCl, NA, NA, 10, NA
mM_EDTA, NA, NA, 0.1, NA") %>% 
  mutate(final_vol = 100*1.1*params$num_samples)
kable(low_te) %>%
  kable_styling() %>%
